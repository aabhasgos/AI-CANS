...
# 1. Insert dossier block correctly
dossier_content = """
...
if "{activeMainTab === 'dossier' &&" not in content:
    # insert it right before the closing div of "absolute inset-0"
    content = content.replace("            </div>\n\n\n          </div>", dossier_content + "            </div>\n\n\n          </div>")


# 2. Add content for Syndicates (communities) and Evidence tabs
syndicates_content = """
...
if "{activeSideTab === 'communities'" not in content:
    # insert before the closing div of the side panel content
    content = content.replace("              </div>\n            )}\n\n          </div>\n        </div>", "              </div>\n            )}\n" + syndicates_content + evidence_content + "\n          </div>\n        </div>")

# 3. Update the report generated logic to be dynamic
report_script_old = "Intelligence_Report_CASE-2024-102.txt"
if report_script_old in content:
    content = content.replace("Intelligence_Report_CASE-2024-102.txt", "Intelligence_Report_CASE.txt")
    content = content.replace("Case: Hawala Syndicate Crackdown (CASE-2024-102)\\n", "Case: ${caseData.title}\\n")
    content = content.replace("Key Mastermind Identified: Rakesh Sharma", "Key Mastermind Identified: ${caseData.suspect.name}")
    content = content.replace("Rakesh Sharma is directly linked to phone number +91 9876543210.\\n", "${caseData.suspect.name} is the primary target.\\n")
    
# Let's just fix the report generation blob fully
new_report = """
                const file = new Blob([
...
"""
content = re.sub(r"const file = new Blob\(\[\n.*?END OF REPORT\"\n\s*\], \{type: 'text\/plain'\}\);", new_report.strip(), content, flags=re.DOTALL)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print("Blank tabs fixed.")
